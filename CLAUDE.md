...
WebXR playground for Meta Quest 3. Three.js + WebXR + cannon-es. **ES modules, no build step.** Named scenes, switchable in-headset. **The one active scene is "The Threshold Cathedral"** (a grounded liminal-horror promenade); the earlier experiment scenes are archived (commented out in `main.js`, files kept for reference).
...
- `src/scenes/thresholdCathedral.js` — **the active scene**: "The Threshold Cathedral", a grounded liminal-horror promenade through seven monumental zones toward a ~300 m door. Wires `createLocomotion` + the `src/cathedral/*` systems + audioscape; ray-trigger nudges doors. `createThresholdCathedral(ctx)`.
...
- **Threshold Cathedral has no collision.** Walls and the balcony void don't stop you; `createLocomotion` clamps `y ≥ 0`, so the abyss is purely visual. The intended route is straight forward (−Z) toward the far door.
- **Its non-Euclidean tricks move geometry / fog / opacity only — never the camera or `dolly`** (VR comfort). Door nudges and interior reshuffles act **only on doors currently off-screen** (a `view.dir · dirToDoor` gaze check), so you never see the snap; the same check makes the distant silhouettes fade out when you look straight at them.
